"""Setup module for Andor Spectrograph SDK.
...
_package_name = 'pyAndorSpectrograph'
...
version_ns = {}
with open(os.path.join(here, _package_name, '_version.py')) as f:
    exec(f.read(), {}, version_ns)
    
setup(
    name=_package_name,
    version=version_ns['__version__'],
    description='Provides a wrapper for the Andor Spectrograph API',
    ...
    install_requires=['cffi', 'numpy==1.19.3'],
    zip_safe=False
)
...
package_lib_path = os.path.join(destination_path, _package_name, 'libs')
